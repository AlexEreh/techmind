...
func (_q *DocumentQuery) QueryCompany() *CompanyQuery {
...
func (_q *DocumentQuery) QueryFolder() *FolderQuery {
...
func (_q *DocumentQuery) QuerySender() *SenderQuery {
...
// QueryDocumentTags chains the current query on the "document_tags" edge.
func (_q *DocumentQuery) QueryDocumentTags() *DocumentTagQuery {
	query := (&DocumentTagClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *DocumentQuery) WithCompany(opts ...func(*CompanyQuery)) *DocumentQuery {
	query := (&CompanyClient{config: _q.config}).Query()
...
func (_q *DocumentQuery) WithFolder(opts ...func(*FolderQuery)) *DocumentQuery {
	query := (&FolderClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *DocumentQuery) WithSender(opts ...func(*SenderQuery)) *DocumentQuery {
	query := (&SenderClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithDocumentTags tells the query-builder to eager-load the nodes that are connected to
// the "document_tags" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *DocumentQuery) WithDocumentTags(opts ...func(*DocumentTagQuery)) *DocumentQuery {
	query := (&DocumentTagClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withDocumentTags = query
	return _q
}
...
//		GroupBy(document.FieldCompanyID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Document.Query().
//		Select(document.FieldCompanyID).
//		Scan(ctx, &v)
...
func (_q *DocumentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Document, error) {
	...
	if query := _q.withCompany; query != nil {
		if err := _q.loadCompany(ctx, query, nodes, nil,
			func(n *Document, e *Company) { n.Edges.Company = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withFolder; query != nil {
		if err := _q.loadFolder(ctx, query, nodes, nil,
			func(n *Document, e *Folder) { n.Edges.Folder = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withDocumentTags; query != nil {
		if err := _q.loadDocumentTags(ctx, query, nodes,
			func(n *Document) { n.Edges.DocumentTags = []*DocumentTag{} },
			func(n *Document, e *DocumentTag) { n.Edges.DocumentTags = append(n.Edges.DocumentTags, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *DocumentQuery) loadCompany(ctx context.Context, query *CompanyQuery, nodes []*Document, init func(*Document), assign func(*Document, *Company)) error {
...
func (_q *DocumentQuery) loadFolder(ctx context.Context, query *FolderQuery, nodes []*Document, init func(*Document), assign func(*Document, *Folder)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Document)
	for i := range nodes {
...
func (_q *DocumentQuery) loadSender(ctx context.Context, query *SenderQuery, nodes []*Document, init func(*Document), assign func(*Document, *Sender)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Document)
	for i := range nodes {
...
func (_q *DocumentQuery) loadDocumentTags(ctx context.Context, query *DocumentTagQuery, nodes []*Document, init func(*Document), assign func(*Document, *DocumentTag)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(document.DocumentTagsTable)
		s.Join(joinT).On(s.C(documenttag.FieldID), joinT.C(document.DocumentTagsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(document.DocumentTagsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(document.DocumentTagsPrimaryKey[0]))
	...
	neighbors, err := withInterceptors[[]*DocumentTag](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "document_tags" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (_q *DocumentQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withCompany != nil {
			_spec.Node.AddColumnOnce(document.FieldCompanyID)
		}
