...
func (_q *CompanyUserQuery) QueryCompany() *CompanyQuery {
...
func (_q *CompanyUserQuery) FirstID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (_q *CompanyUserQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
func (_q *CompanyUserQuery) OnlyID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (_q *CompanyUserQuery) OnlyIDX(ctx context.Context) uuid.UUID {
...
func (_q *CompanyUserQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (_q *CompanyUserQuery) IDsX(ctx context.Context) []uuid.UUID {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *CompanyUserQuery) WithCompany(opts ...func(*CompanyQuery)) *CompanyUserQuery {
	query := (&CompanyClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(companyuser.FieldUserID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.CompanyUser.Query().
//		Select(companyuser.FieldUserID).
//		Scan(ctx, &v)
...
func (_q *CompanyUserQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*CompanyUser, init func(*CompanyUser), assign func(*CompanyUser, *User)) error {
	...
	nodeids := make(map[uuid.UUID][]*CompanyUser)
	for i := range nodes {
		fk := nodes[i].UserID
...
func (_q *CompanyUserQuery) loadCompany(ctx context.Context, query *CompanyQuery, nodes []*CompanyUser, init func(*CompanyUser), assign func(*CompanyUser, *Company)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*CompanyUser)
	for i := range nodes {
...
func (_q *CompanyUserQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(companyuser.Table, companyuser.Columns, sqlgraph.NewFieldSpec(companyuser.FieldID, field.TypeUUID))
	_spec.From = _q.sql
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withUser != nil {
			_spec.Node.AddColumnOnce(companyuser.FieldUserID)
		}
		if _q.withCompany != nil {
			_spec.Node.AddColumnOnce(companyuser.FieldCompanyID)
		}
	}
...
func (_q *CompanyUserQuery) Modify(modifiers ...func(s *sql.Selector)) *CompanyUserSelect {
...
// Modify adds a query modifier for attaching custom logic to queries.
func (_s *CompanyUserSelect) Modify(modifiers ...func(s *sql.Selector)) *CompanyUserSelect {
