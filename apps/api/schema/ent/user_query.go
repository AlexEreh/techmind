...
func (_q *UserQuery) QueryCompanyUsers() *CompanyUserQuery {
...
func (_q *UserQuery) WithCompanyUsers(opts ...func(*CompanyUserQuery)) *UserQuery {
	query := (&CompanyUserClient{config: _q.config}).Query()
...
func (_q *UserQuery) loadCompanyUsers(ctx context.Context, query *CompanyUserQuery, nodes []*User, init func(*User), assign func(*User, *CompanyUser)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(companyuser.FieldUserID)
	}
	query.Where(predicate.CompanyUser(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.CompanyUsersColumn), fks...))
	}))
